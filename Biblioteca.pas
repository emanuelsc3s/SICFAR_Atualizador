...
Uses
   ...
   IBX.IBCustomDataSet, IBX.IBTable, Menus, RzDBEdit, RzEdit, Registry, StrUtils,
   ...
   procedure DesabilitaDBEdit(Form: TForm);
   procedure HabilitaDBEdit(Form: TForm);
...
begin
  ...
  begin
    ...
    try
      Result := FileStream.Size;  // Obtém o tamanho do arquivo
    finally
      FileStream.Free;
    end;
  end;
...
begin
  try
    ...
    for j := 1 to OutF.Sheets.Count do
    begin
      // Verificação do índice da planilha
      if j > OutF.Sheets.Count then
      begin
        ShowMessage('Índice inválido para a planilha ' + IntToStr(j));
        Exit;
      end;

      Sheet := OutF.WorkSheets[j];

      // Verificação adicional para garantir que a planilha foi selecionada corretamente
      if VarIsNull(Sheet) or VarIsEmpty(Sheet) then
      begin
        ShowMessage('Não foi possível acessar a planilha ' + IntToStr(j));
        Exit;
      end;

      if Length(SValue) < 250 then
      begin
        Sheet.Cells.Replace(What := Slabel, Replacement := SValue, LookAt := 2, SearchOrder := 1, MatchCase := False);
      end
      else
      begin
        // Excel .replace fails on string with length >250 so replace it in few steps
        ...
        begin
          if i + 200 - 1 < Length(SValue) then
            Sheet.Cells.Replace(What := Slabel, Replacement := Copy(SValue, i, 200) + SLabel, LookAt := 2, SearchOrder := 1, MatchCase := False)
          else
            Sheet.Cells.Replace(What := Slabel, Replacement := Copy(SValue, i, 200), LookAt := 2, SearchOrder := 1, MatchCase := False);
          i := i + 200;
        end;
      end;
    end;
    ...
    on E: Exception do
      ShowMessage('Error: Label [' + SLabel + '] ' + E.Message);
  end;
end;
...
function RenomearArquivo(caminhoOriginal, novoNome: string): string;
var
  diretorio, extensao, novoCaminho: string;
begin
  // Extrai o diretório do arquivo original
  diretorio := ExtractFilePath(caminhoOriginal);
  // Extrai a extensão do arquivo original
  extensao := ExtractFileExt(caminhoOriginal);
  // Compõe o novo caminho com o diretório original, o novo nome e a extensão original
  novoCaminho := diretorio + novoNome + extensao;

  // Tenta renomear o arquivo
  RenameFile(caminhoOriginal, novoCaminho);
  Result := novoCaminho;
end;
...
begin
  ...
  while not qConfig.Eof do
    begin
      try
      if VarIsClear(vlExcel.Workbooks[1].Sheets[qConfig.FieldByName('PAGINA').AsInteger]) then
        raise Exception.Create('A aba ' + qConfig.FieldByName('PAGINA').AsString + ' especificada não existe no arquivo do Excel.');
      ...
      vlExcel.Workbooks[1].Sheets[qConfig.FieldByName('PAGINA').AsInteger].PageSetup.PrintArea  := '$' + qConfig.FieldByName('COLUNA_I').AsString +'$' + qConfig.FieldByName('LINHA_I').AsString +
        ...
        begin
          ShowMessage('Erro ao configurar área de impressão: ' + E.Message);
          Exit; // Aborta a execução da função
        end;
      end;
...
begin
  ...
  MsWord.ActiveDocument.SaveAs(prOrigem);
  MsWord.ActiveDocument.Close(False, EmptyParam, EmptyParam);
  MsWord.Quit;
...
begin
  ...
  if Form.Components[i] is TRzDBEdit then
    (Form.Components[i] as TRzDBEdit).TabStop := False;
end;
...
begin
  for i := 0 to Form.ComponentCount - 1 do
  if Form.Components[i] is TRzDBEdit then
    (Form.Components[i] as TRzDBEdit).Enabled := True;
end;
