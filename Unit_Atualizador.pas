...
uses
  ...
  Vcl.ExtCtrls, Vcl.ComCtrls, RzPrgres, RzPanel, dxGDIPlusClasses;
...
var
  ...
  LSistema : string;
begin
  LSistema := ExtractFileName(Application.ExeName);

  AdicionaLog('Iniciando Atualização');
  try
    AdicionaLog('Tentando Conexão com:'+dm1.Cliente.Host);
    dm1.Cliente.Connect; //Tenta se conectar
    dm1.Cliente.IOHandler.WriteLn(Usuario);
    AdicionaLog('Conexão estabelecida com sucesso');
    ProgressBar1.Max := StrToInt(dm1.Cliente.IOHandler.ReadLn);
  ...
  try
    with dm1.Cliente do
      begin
        ...
        IOHandler.WriteLn(LSistema +'_' + 'arquivos#'); //manda o comando para o servidor. (deixe um espaço após o texto.)
  ...
  for I:= 0 to Versoes.Count-1 do
    begin
      ...
      try
        with dm1.Cliente do
          begin
            IOHandler.WriteLn(LSistema +'_' +'get#'+Arquivo);
            ...
            if Arquivo = LSistema then
              begin
                RenameFile(LSistema, Copy(LSistema, 1, pos('.',LSistema)-1) + '-' + Get_Versao +'.exe');
                StreamGet.SaveToFile(Caminho + LSistema);
  ...
  dm1.Cliente.Disconnect;

  AdicionaLog('Atualização Terminada.');

  vlQuerySIC := TIBQuery.Create(self);
  with vlQuerySIC do
    begin
  ...
  ShellExecute(0,Nil,PChar(GetCurrentDir +'\' +LSistema),'', Nil, SW_SHOWNORMAL);
...
//      Open;
//    end;
//  dm1.Cliente.Host := vlQuerySIC.FieldByName('CONTEUDO').AsString;
//
//  with vlQuerySIC do
//    begin
//      Close;
...
//      Open;
//    end;
//  dm1.Cliente.Port := vlQuerySIC.FieldByName('CONTEUDO').AsInteger;
//
//  with vlQuerySIC do
//    begin
//      Close;
...
//      Open;
//    end;
//  Caminho := ExtractFilePath(Application.ExeName);// Application. vlQuerySIC.FieldByName('CONTEUDO').AsString;
//
//  with vlQuerySIC do
//    begin
//      Close;
...
//  NovoArquivo := vlQuerySIC.FieldByName('CONTEUDO').AsString;
  ...
  try
    dm1.Cliente.Host := ArqIni.ReadString ('PORTAUPDATE', 'Host',        dm1.Cliente.Host);
    dm1.Cliente.Port := ArqIni.ReadInteger('PORTAUPDATE', 'Porta',       dm1.Cliente.Port);
    Caminho          := ArqIni.ReadString ('PORTAUPDATE', 'Caminho',     Caminho);
    NovoArquivo      := ArqIni.ReadString ('PORTAUPDATE', 'NovoArquivo', NovoArquivo);
  finally
    ArqIni.Free;
  end;
