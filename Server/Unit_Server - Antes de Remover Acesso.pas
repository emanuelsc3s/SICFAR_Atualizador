          ...
          if sr.Name = 'SICFAR.exe' then
            ...
            begin
              if Copy(sr.Name, 1, Pos('.', sr.Name, 1)) = '' then
                begin
                  dir := sr.Name + '\';
                  IDir := FindFirst(Caminho+dir+'*.*', faAnyFile, SRDir);
                  while IDir = 0 do
                    begin
                      if ((SRDir.Name <> '.') and (SRDir.Name <> '..')) then
                        begin
                          if SRDir.Name = 'SICFAR.exe' then
                            Memo_Arquivos.Lines.Add(dir+SRDir.Name+'='+GetBuildInfo(Caminho+dir+SRDir.Name))
                          else
                            Memo_Arquivos.Lines.Add(dir+SRDir.Name+'='+DateTimeToStr(FileDateToDateTime(FileAge(Caminho+dir+SRDir.Name))));
                ...
                Memo_Arquivos.Lines.Add(sr.Name+'='+DateTimeToStr(FileDateToDateTime(FileAge(Caminho+sr.Name))));
            end;
